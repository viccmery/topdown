...
df1 = pd.read_csv('/Volumes/lab-windingm/home/users/cochral/LRS/AttractionRig/analysis/social-isolation/pseudo-n10/socially-isolated/closest_contacts_1mm.csv')
...
for interaction, sub in grouped.groupby("Closest Interaction Type"):
    ps = sub.loc[sub["condition"] == "PSEUDO", "count"]
    si = sub.loc[sub["condition"] == "SI", "count"]

    # skip if one group is missing (safety)
    if ps.empty or si.empty:
        continue

    u, p = mannwhitneyu(ps, si, alternative="two-sided")
    ...
    results.append({
        ...
        "u_stat": u,
        "p_raw": p,
        "median_PSEUDO": ps.median(),
        "median_SI": si.median(),
        "delta_median": si.median() - ps.median()
    })
...
plt.savefig('/Users/cochral/repos/behavioural-analysis/plots/lrs_paper/GHxSI/interaction_type-closestnode_n10_pseudo.pdf', 
