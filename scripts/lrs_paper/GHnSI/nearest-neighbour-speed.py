...
baseline = (
    df[(df['bin_right'] >= 10) & (df['bin_right'] < 20)]
      .groupby(['filename'], as_index=False)['speed']
      .mean()
      .rename(columns={'speed': 'mean_speed_10_20'})
)

df = df.merge(baseline, on='filename', how='left')
df = df.dropna(subset=['mean_speed_10_20']).copy()
df['speed_norm'] = df['speed'] / df['mean_speed_10_20']


df_plot = (
    df.groupby(['social_experience', 'filename', 'bin_right'], as_index=False)['speed_norm']
      .mean()
)
...
sns.despine()
plt.savefig('/Users/cochral/repos/behavioural-analysis/plots/lrs_paper/GHnSI/head_min_dist-speed.pdf', format='pdf', bbox_inches='tight')
